"""Build a graphify graph from an already-extracted AST. Run from a graphs/<name> dir."""
import json, sys
...
if not to_json(G, communities, 'graphify-out/graph.json'):
    print('ERROR: refused to shrink graph.json'); raise SystemExit(1)
Path('graphify-out/GRAPH_REPORT.md').write_text(
    generate(G, communities, cohesion, labels, gods, surprises, detection, tokens, ROOT,
